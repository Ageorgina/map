...
export class MapaSeccionesAdminComponent implements OnInit, OnDestroy {
  loading = true;
  datosSecciones = [];
  headers = [];
  prueba: any;
  ...
  constructor(private mapaSrv: MapasService, private route: ActivatedRoute, private ngZone: NgZone) {
    ...
    this.traerData(this.id).finally(() => {
      this.mapaSrv.getCoordenadasSecciones(this.id, this.estado).subscribe(csvInfo => {
        this.construirMapa(csvInfo).finally(() => this.loading = false);
      });
    });
